...
interface SelectProps {
  ...
  options: { key: string; value: string }[] | string[]; disabled?: boolean; id?: string;
}
const Select = memo(function Select({ label, value, onChange, options, disabled = false, id }: SelectProps) {
...
function LeadFormContent() {
  ...
  useEffect(() => {
    ...
    const fetchClients = async () => {
      try {
        const res = await apiFetch('/api/clients?pageSize=200');
        if (!mounted) return;
        if (res.ok) {
          const data = await res.json();
  ...
  // ============================================================================
  // 🔹 FETCH LEAD DATA (EDIT/VIEW MODE)
  // ============================================================================
  useEffect(() => {
    ...
    const fetchLeadData = async () => {
      try {
        ...
        if (mounted) {
          ...
          if (data.payment) {
            setPayment({
              totalAmount: data.payment.totalAmount?.toString() || '', commissionAmount: data.payment.commissionAmount?.toString() || '',
              commissionName: data.payment.commissionName || '', commissionDate: data.payment.commissionDate || '',
              grnNumber: data.payment.grnNumber || '', grnAmount: data.payment.grnAmount?.toString() || '',
        ...
        if (mounted) setFormError('Failed to load lead data. Please try again.');
      }
    };
    fetchLeadData();
    return () => { mounted = false; };
  ...
  // ============================================================================
  // 🔹 STABLE STATE UPDATE HANDLERS
  // ============================================================================
  const updateLead = useCallback((field: keyof LeadFormData, value: string) => {
    setLead(prev => prev[field] === value ? prev : { ...prev, [field]: value });
  }, []);
  ...
  const saveLead = useCallback(async () => {
    ...
    try {
      ...
      let response;
      if (currentLeadId) {
        response = await apiFetch('/api/leads', { method: 'PUT', body: JSON.stringify({ id: currentLeadId, ...payload }) });
      } else {
        response = await apiFetch('/api/leads', { method: 'POST', body: JSON.stringify(payload) });
      }
      if (!response.ok) {
  ...
  const savePayment = useCallback(async () => {
    ...
    try {
      const response = await apiFetch('/api/payments', {
        method: 'POST',
        body: JSON.stringify({
          leadId: currentLeadId, totalAmount: parseFloat(payment.totalAmount) || 0, commissionAmount: parseFloat(payment.commissionAmount) || 0,
