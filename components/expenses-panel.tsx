...
// ============================================================================
// 🔹 Expenses Panel — commission payouts (money going OUT) per lead.
// Lives inside the Payment Statement page as the "Expenses" tab. Mirrors the
// Statement table's look, but each row is one lead's commission expense pulled
// from lead.payment (commissionName / commissionDate / commissionAmount).
//
// Columns: Sr. No · Lead Date · Token No · Lead Name · Back Work Account ·
//          Commission Date · Commission Name · Commission Amount.
//
...
// Flatten leads → expense rows. One row per lead that actually has a commission
// (a name or an amount). Newest commission date on top.
const buildRows = (leads: ExpenseLead[]): ExpenseRow[] => {
  const rows: ExpenseRow[] = [];
  for (const lead of leads) {
    const p = lead.payment;
    const commissionName = p?.commissionName?.trim() || '';
    const commissionAmount = toNum(p?.commissionAmount);
    // Only include leads that carry a real commission expense.
    if (!commissionName && commissionAmount <= 0) continue;
    rows.push({
      leadId: lead.id,
      date: p?.commissionDate || '',
  ...
  return (
    <>
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
        💸 <strong>Expenses (Commission Payouts):</strong> Every lead&apos;s commission expense, newest commission date on top.
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-xs text-slate-500 mb-1">Total Commission (shown)</p>
          <p className="text-lg font-semibold text-amber-600 flex items-center gap-1">
            <IndianRupee className="w-4 h-4" /> {formatINR(total)}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-[#00843d] via-[#0d9488] to-[#0e7490] border-b border-[#00622d]">
              <tr>
                ...
                <th className={`${th} text-right`}>Commission Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan={COLS} className="px-4 py-10 text-center text-slate-400">No commission expenses found.</td>
                </tr>
              ) : (
                pagedRows.map((r, i) => (
                  <tr key={`${r.leadId}-${i}`} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className={`${td} text-right font-semibold text-amber-600`}>{formatINR(r.commissionAmount)}</td>
                  </tr>
                ))
              )}
            </tbody>
            {rows.length > 0 && (
              <tfoot className="bg-slate-50 border-t border-slate-200">
                <tr>
                  ...
                  <td className={`${td} text-right font-bold text-amber-600`}>{formatINR(total)}</td>
                </tr>
              </tfoot>
            )}
          </table>
